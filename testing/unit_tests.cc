...
TEST(EventData, write_evt_data)
{
    ...
    }
    {
        test_ed.lock_data_vectors();
        const auto &evt_data_vec{test_ed.get_evt_vector_ref()};
        ASSERT_EQ(evt_data_vec.size(), NUM_ELEMENTS) << "Internal event data size mismatch when testing relative timestamps.";
        for(int32_t i{0}; i < NUM_ELEMENTS; ++i)
        {
            glm::vec4 evt_datum{evt_data_vec[i]};
            EXPECT_EQ(evt_datum, glm::vec4(i, i, i, 0)) << "Out of order writing of event data at " << i << " when testing relative timestamps."; 
        }
    ...
    test_ed.write_evt_data(out_of_order_datum);
    {
        test_ed.lock_data_vectors();
        const auto &evt_data_vec{test_ed.get_evt_vector_ref()};
        EXPECT_EQ(evt_data_vec.size(), 1) << "Out of order writing of event data did not clear vector"; // Should have cleared vector before inserting
        test_ed.unlock_data_vectors();
    }
    ...
    // Test event data is written in order
    {
        ...
        {
            glm::vec4 evt_datum{evt_data_vec[i]};
            EXPECT_EQ(evt_datum, glm::vec4(i, i, i, 0)) << "Out of order writing of event data at " << i << " after reset."; 
        }
...
TEST(EventData, write_frame_data)
{
    ...
    }
    {
        test_ed.lock_data_vectors();
        const auto &frame_data_vec{test_ed.get_frame_vector_ref()};
        ASSERT_EQ(frame_data_vec.size(), NUM_ELEMENTS) << "Internal frame data size mismatch when testing relative timestamps.";
        for(int32_t i{0}; i < NUM_ELEMENTS; ++i)
        {
            auto curr_timestamp{frame_data_vec[i].second};
            EXPECT_EQ(curr_timestamp, i) << "Out of order writing of frame data at " << i << " when testing relative timestamps."; 
        }
    ...
    test_ed.write_evt_data(evt_datum);
    test_ed.write_frame_data(out_of_order_datum); // Write again so write succeeds with timestamps relative to event data
    {
        test_ed.lock_data_vectors();
        const auto &frame_data_vec{test_ed.get_frame_vector_ref()};
        EXPECT_EQ(frame_data_vec.size(), 1) << "Out of order writing of event data did not clear vector"; // Should have cleared vector before inserting
        test_ed.unlock_data_vectors();
    }
...
TEST(EventData, get_earliest_evt_timestamp)
{
    ...
    test_ed.write_evt_data(evt_datum_absolute);
    EXPECT_EQ(test_ed.get_earliest_evt_timestamp(), 123) << "Earliest event timestamp mismatch after writing another datum.";

    EventData::EventDatum evt_datum_less{.x = 0, .y = 0, .timestamp = 3, .polarity = 0};
    test_ed.write_evt_data(evt_datum_less);
    EXPECT_EQ(test_ed.get_earliest_evt_timestamp(), 3) << "Earliest event timestamp mismatch after out of order writing.";
...
TEST(EventData, get_event_index_from_timestamp)
{
...

    {
        ...
        EXPECT_EQ(bad_index1, -1) << "Expected -1 for non-existent timestamp (no timestamp greater than or equal to provided exist).";
