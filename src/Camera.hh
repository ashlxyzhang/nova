...
class Camera
{
    ...
    public:
         ...
         */
        Camera(glm::vec3 orbit_center = glm::vec3(0.0f, 0.0f, 0.0f), float orbit_distance = 5.0f, float fov = 45.0f,
               float aspect_ratio = 16.0f / 9.0f, float near_plane = 0.1f, float far_plane = 1000.0f)
            : m_world_up(0.0f, 1.0f, 0.0f), m_yaw(0.0f), m_pitch(0.0f), m_orbit_center(orbit_center),
              m_orbit_distance(orbit_distance), m_fov(fov), m_aspect_ratio(aspect_ratio), m_near_plane(near_plane),
              m_far_plane(far_plane), m_mouse_sensitivity(0.1f), m_zoom_sensitivity(2.0f), m_min_fov(1.0f),
              m_max_fov(120.0f)
        {
            updateOrbitPosition();
        }
