...
void BackendOptimization::sloveProblem()
{
    ...
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        timePoint tt = (*pDepthPoints_)[dqvDepthPoints_size - WINDOW_SIZE - 1 + i].timestamp_;
...
void BackendOptimization::vector2Double(esvo2_core::container::TimeSurfaceHistory::iterator ts_obs,
                                        esvo2_core::container::TimeSurfaceHistory::iterator last_obs,
                                        double para_Pose[][7], double para_SpeedBias[][9], int i)
{
                                    ...
                                    int i)
{
    ...
    else
    {
        ...
        T_window0_i =
            T_i_c_ * T_wopt_window0_.inverse() * T_wopt_last * T_wori_last.inverse() * T_wori_c * T_i_c_.inverse();
...
void BackendOptimization::double2Vector(double para_Pose[][7], double para_SpeedBias[][9])
{
    ...
    {
        Rs[i] =
            R_wopt_window0 * RIC_.inverse() *
            Eigen::Quaterniond(para_Pose[i][6], para_Pose[i][3], para_Pose[i][4], para_Pose[i][5]).toRotationMatrix() *
            RIC_;
        Ps[i] = R_wopt_window0 * RIC_.inverse() *
                    Eigen::Vector3d(para_Pose[i][0] - para_Pose[0][0], para_Pose[i][1] - para_Pose[0][1],
...
void BackendOptimization::RefineGravity(Eigen::Vector3d &g, Eigen::VectorXd &x)
{
    ...
    for (int k = 0; k < 4; k++)
    {
        ...
        for (; i < WINDOW_SIZE; i++)
        {
            ...
            tmp_b.block<3, 1>(0, 0) = pre_integrations[i]->delta_p + Rs[i].transpose() * Rs[i + 1] * TIC_ - TIC_ -
                                      Rs[i].transpose() * dt * dt / 2 * g0;

            tmp_A.block<3, 3>(3, 0) = -Eigen::Matrix3d::Identity();
            tmp_A.block<3, 3>(3, 3) = Rs[i].transpose() * Rs[i + 1];
            tmp_A.block<3, 2>(3, 6) = Rs[i].transpose() * dt * Eigen::Matrix3d::Identity() * lxly;
            tmp_b.block<3, 1>(3, 0) =
                pre_integrations[i + 1]->delta_v - Rs[i].transpose() * dt * Eigen::Matrix3d::Identity() * g0;
    ...
    if (frame_count >= WINDOW_SIZE)
    {
        ...
        if (bUSE_IMU_)
        {
            ...
            pre_integrations[WINDOW_SIZE] =
                new IntegrationBase{acc_0, gyr_0, Bas[WINDOW_SIZE], Bgs[WINDOW_SIZE], g_optimal};
        }
    }
    if (frame_count < WINDOW_SIZE)
        frame_count++;
