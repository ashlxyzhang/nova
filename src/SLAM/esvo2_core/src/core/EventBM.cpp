...
esvo2_core::core::EventBM::EventBM(esvo2_core::CameraSystem::Ptr camSysPtr, size_t numThread, bool bSmoothTS,
                                   size_t patch_size_X, size_t patch_size_Y, size_t min_disparity, size_t max_disparity,
                                   size_t step, double ZNCC_Threshold, bool bUpDownConfiguration)
    : camSysPtr_(camSysPtr), sb_(Sobel(3)), NUM_THREAD_(numThread), patch_size_X_(patch_size_X),
      patch_size_Y_(patch_size_Y), min_disparity_(min_disparity), max_disparity_(max_disparity), step_(step),
      ZNCC_Threshold_(ZNCC_Threshold), bUpDownConfiguration_(bUpDownConfiguration)
{
    ZNCC_MAX_ = 1.0;
...
void esvo2_core::core::EventBM::resetParameters(size_t patch_size_X, size_t patch_size_Y, size_t min_disparity,
                                                size_t max_disparity, size_t step, double ZNCC_Threshold,
                                                bool bUpDownConfiguration, size_t patch_size_X_2, size_t patch_size_Y_2)
{
...
void esvo2_core::core::EventBM::createMatchProblem(constStampedTimeSurfaceObs *pStampedTsObs,
                                                   StampTransformationMap *pSt_map, std::vector<Event *> *pvEventsPtr)
{
...
void esvo2_core::core::EventBM::createMatchProblemTwoFrames(constStampedTimeSurfaceObs *pStampedTsObs,
                                                            StampTransformationMap *pSt_map,
                                                            std::vector<Event *> *pvEventsPtr,
                                                            std::vector<EventMatchPair> *vEMP_lr)
{
    ...
    for (size_t i = 0; i < vEventsPtr_.size(); i++)
        vpDisparitySearchBound_.push_back(std::make_pair(min_disparity_, max_disparity_));
    T_last_now_ = pStampedTsObs_->second.tr_last_.getTransformationMatrix().inverse() *
                  pStampedTsObs_->second.tr_.getTransformationMatrix();
    num_of_need_ssim_ = 0;
    num_of_success_ssim_ = 0;
}

bool esvo2_core::core::EventBM::match_an_event2(const Event *pEvent, std::pair<size_t, size_t> &pDisparityBound,
                                                esvo2_core::core::EventMatchPair &emPair)
{
    ...
    if (x_rect(0) < 0 || x_rect(0) > camSysPtr_->cam_left_ptr_->width_ - 1 || x_rect(1) < 0 ||
        x_rect(1) > camSysPtr_->cam_left_ptr_->height_ - 1)
    {
    ...
    Eigen::MatrixXd patch_src =
        pStampedTsObs_->second.TS_left_.block(x1_left_top(1), x1_left_top(0), patch_size_Y_, patch_size_X_);
    ...
    // To improve search efficiency, static stereo block matching performs searches at a certain step size and conducts
    // additional searches near points that preliminarily match.
    if (!epipolarSearchingCoarse(min_cost, bestMatch, bestDisp, patch_dst, lowDisparity, upDisparity, step_, x1,
                                 patch_src, bUpDownConfiguration_, searching_or_not, colSum, colSquareSum, mean_l,
                                 Tl_square, Tr, Tr_square))
    {
    ...
    if (step_ > 1)
    {
        if (!epipolarSearchingFine(min_cost, bestMatch, bestDisp, patch_dst, searching_radius, x1, patch_src,
                                   bUpDownConfiguration_))
        {
...
bool esvo2_core::core::EventBM::match_an_event(Event *pEvent, std::pair<size_t, size_t> &pDisparityBound,
                                               esvo2_core::core::EventMatchPair &emPair)
{
    ...
    Eigen::MatrixXd patch_dst = Eigen::MatrixXd::Zero(patch_size_Y_, patch_size_X_);
    // coarse searching
    if (!epipolarSearching(min_cost, bestMatch, bestDisp, patch_dst, lowDisparity, upDisparity, step_, x1, patch_src,
                           bUpDownConfiguration_))
    {
        coarseSearchingFailNum_++;
        return false;
    ...
    if (!epipolarSearching(min_cost, bestMatch, bestDisp, patch_dst, fine_searching_start_pos, bestDisp + (step_ - 1),
                           1, x1, patch_src, bUpDownConfiguration_))
    {
...
bool esvo2_core::core::EventBM::match_an_eventTwoFrames(EventMatchPair EMP_lr,
                                                        ...
                                                        esvo2_core::core::EventMatchPair &emPair)
{
    ...
    // extract the template patch in the left time_surface
    Eigen::MatrixXd patch_src =
        pStampedTsObs_->second.TS_left_.block(x1_left_top(1), x1_left_top(0), patch_size_X_2_, patch_size_Y_2_);
    if ((patch_src.array() < 1).count() > 0.95 * patch_src.size())
    {
        infoNoiseRatioLowNum_++;
        return false;
    ...
    if (!epipolarSearchingTwoFrames(min_cost, bestMatch, bestDisp, patch_dst, lowDisparity, upDisparity, 1, x1,
                                    patch_src, dx, dy, emPair.costs_, fine_search, bUpDownConfiguration_))
    {
        coarseSearchingFailNum_++;
        return false;
    }
    ...
    if (min_cost <= ZNCC_Threshold_ * 3.00)
    {
        ...
        emPair.x_right_ << EMP_lr.x_left_(0) - camSysPtr_->baseline_ * camSysPtr_->cam_left_ptr_->P_(0, 0) / depth,
            EMP_lr.x_left_(1);
        emPair.trans_ = emPair.trans_ = pStampedTsObs_->second.tr_;
        emPair.invDepth_ = 1.0 / depth;
        emPair.cost_ = min_cost;
...
bool esvo2_core::core::EventBM::epipolarSearching(double &min_cost, Eigen::Vector2i &bestMatch, size_t &bestDisp,
                                                  Eigen::MatrixXd &patch_dst, size_t searching_start_pos,
                                                  size_t searching_end_pos, size_t searching_step, Eigen::Vector2i &x1,
                                                  Eigen::MatrixXd &patch_src, bool bUpDownConfiguration)
{
    ...
    for (size_t disp = searching_start_pos; disp <= searching_end_pos; disp += searching_step)
    {
        ...
        patch_dst =
            pStampedTsObs_->second.TS_right_.block(x2_left_top(1), x2_left_top(0), patch_size_Y_, patch_size_X_);
...
bool esvo2_core::core::EventBM::epipolarSearchingCoarse(
    ...
    size_t searching_start_pos, size_t searching_end_pos, size_t searching_step, Eigen::Vector2i &x1,
    Eigen::MatrixXd &patch_src, bool bUpDownConfiguration, vector<bool> &searching_or_not, Eigen::VectorXd &colSum,
    Eigen::VectorXd &colSquareSum, double &mean_l, double &Tl_square, double &Tr, double &Tr_square)
{
    double var_l = 0;
    int zncc_num = 0;
    bool bFoundOneMatch = false;
    ...
    for (size_t disp = searching_start_pos; disp <= searching_end_pos; disp += searching_step)
    {
        ...
        if (use_fast_zncc_)
        {
            ...
            cost = zncc_cost_fast(colSum, colSquareSum, patch_src, patch_dst, disp_to_rm, step_to_rm, mean_l, Tl_square,
                                  Tr, Tr_square);
        }
        else
        {
        ...
        {
            for (int i = disp - searching_start_pos - searching_step;
                 i < disp - searching_start_pos + searching_step + 1; i++)
            {
...
bool esvo2_core::core::EventBM::epipolarSearchingFine(double &min_cost, Eigen::Vector2i &bestMatch, size_t &bestDisp,
                                                      Eigen::MatrixXd &patch_dst, vector<size_t> &searching_radius,
                                                      Eigen::Vector2i &x1, Eigen::MatrixXd &patch_src,
                                                      bool bUpDownConfiguration)
{
    bool bFoundOneMatch = false;
    double mean_l = patch_src.array().mean();
    double var_l =
        (patch_src.array() * patch_src.array()).sum() - patch_src.rows() * patch_src.cols() * mean_l * mean_l;
...
bool esvo2_core::core::EventBM::epipolarSearchingTwoFrames(double &min_cost, Eigen::Vector2i &bestMatch, int &bestDisp,
                                                           Eigen::MatrixXd &patch_dst, int searching_start_pos,
                                                           int searching_end_pos, size_t searching_step,
                                                           Eigen::Vector2i &x1, Eigen::MatrixXd &patch_src, double dx,
                                                           double dy, std::vector<double> &costs,
                                                           std::vector<bool> &fine_search, bool bUpDownConfiguration)
{
    ...
    if (!use_zncc)
    {
        ...
        p1 = pStampedTsObs_->second.img_left_(rect1);
    }
    double mean_l = patch_src.array().mean();
    double var_l =
        (patch_src.array() * patch_src.array()).sum() - patch_src.rows() * patch_src.cols() * mean_l * mean_l;
    ...
    for (int disp = searching_start_pos; disp <= searching_end_pos; disp += searching_step)
    {
        ...
        else
        {
            ...
            p2 = pStampedTsObs_->second.img_last_(rect2);
...
void esvo2_core::core::EventBM::match_all_SingleThread(std::vector<EventMatchPair> &vEMP)
{
    TicToc t;
    coarseSearchingFailNum_ = 0;
    fineSearchingFailNum_ = 0;
...
void esvo2_core::core::EventBM::match_all_SingleThreadTwoFrames(std::vector<EventMatchPair> &vEMP,
...
void esvo2_core::core::EventBM::match_all_HyperThreadTwoFrames(std::vector<EventMatchPair> &vEMP,
                                                               std::vector<EventMatchPair> &vEMP_fail)
{
    TicToc t;
    t.tic();
...
bool esvo2_core::core::EventBM::isValidPatch(Eigen::Vector2i &x, Eigen::Vector2i &left_top, int size_y, int size_x)
{
    int wx = (size_x - 1) / 2;
    int wy = (size_y - 1) / 2;
    ...
    if (left_top(0) < 1 || left_top(1) < 1 || right_bottom(0) >= camSysPtr_->cam_left_ptr_->width_ - 1 ||
        right_bottom(1) >= camSysPtr_->cam_left_ptr_->height_ - 1)
        return false;
    return true;
}

void esvo2_core::core::EventBM::match2(EventBM::Job &job)
{
...
void esvo2_core::core::EventBM::match2_TwoFrames(EventBM::Job2 &job)
{
    size_t i_thread = job.i_thread_;
...
void esvo2_core::core::EventBM::match(EventBM::Job &job)
{
    size_t i_thread = job.i_thread_;
    size_t totalNumEvents = job.pvEventPtr_->size();
    job.pvEventMatchPair_->reserve(totalNumEvents / NUM_THREAD_ + 1);
...
double esvo2_core::core::EventBM::zncc_cost(Eigen::MatrixXd &patch_left, Eigen::MatrixXd &patch_right, bool normalized)
{
    double cost;
    if (!normalized)
    {
...
double esvo2_core::core::EventBM::zncc_cost_fast(Eigen::VectorXd &colSum, Eigen::VectorXd &colSquareSum,
                                                 Eigen::MatrixXd &patch_left, Eigen::MatrixXd &patch_right,
                                                 int &disp_to_rm, int &step_to_rm, double &mean_l, double &Tl_square,
                                                 double &Tr, double &Tr_square)
{
    ...
    for (int i = 0; i < step_to_rm; i++)
    {
        ...
        {
            Tr = Tr - colSum[colSum.size() - 1 - disp_to_rm] +
                 colSum[colSum.size() - 1 - disp_to_rm - patch_right.cols()];
            Tr_square = Tr_square - colSquareSum[colSum.size() - 1 - disp_to_rm] +
                        colSquareSum[colSum.size() - 1 - disp_to_rm - patch_right.cols()];
            disp_to_rm++;
        }
    }
...
double esvo2_core::core::EventBM::zncc_cost2(Eigen::MatrixXd &patch_left, Eigen::MatrixXd &patch_right, double &var_l,
                                             double &mean_l)
{
    ...
    var_r =
        (patch_right.array() * patch_right.array()).sum() - patch_right.rows() * patch_right.cols() * mean_r * mean_r;
    if (var_l * var_r == 0)
        cost = 0;
    else
        cost = cov / sqrt(var_l * var_r);
