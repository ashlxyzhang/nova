...
{
        EventMessageEditor(double frequency, std::string &messageTopic) : bFirstMessage_(true)
        {
            eArray_ = EventArray();
        ...
        void insertEvent(Event &e, rosbag::Bag *bag)
        {
            if (bFirstMessage_)
            {
                resetBuffer(e.ts);
...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 0; i < topics.size(); i++)
    {
        ...
        {
            EventArray::ConstPtr msg = m.instantiate<EventArray>();
            eArrayEditor.resetArraySize(msg->width, msg->height);
            // message loop
            for (Event e : msg->events)
            {
                nums++;
                eArrayEditor.insertEvent(const_cast<Event &>(e), &bag_dst);
            }
        }
    }
